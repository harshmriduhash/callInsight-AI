...
export async function analyzeObjections(transcription: string): Promise<ObjectionAnalysis> {
  try {
...
Transcription: "${transcription}"`
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert sales objection handler. Always respond with valid JSON only.',
        },
        {
          role: 'user',
          content: prompt,
