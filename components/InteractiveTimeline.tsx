...
import { Clock, TrendingUp, TrendingDown, AlertCircle, CheckCircle, XCircle } from 'lucide-react'
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ReferenceLine } from 'recharts'

interface TimelinePoint {
  timestamp: string
  type: 'emotion' | 'silence' | 'interruption' | 'closing' | 'rapport' | 'objection'
  label: string
  description: string
...
export default function InteractiveTimeline({
  ...
  function getPointColor(type: string, importance: string) {
    const colors: { [key: string]: { [key: string]: string } } = {
      emotion: { high: 'bg-blue-500', medium: 'bg-blue-400', low: 'bg-blue-300' },
      silence: { high: 'bg-gray-500', medium: 'bg-gray-400', low: 'bg-gray-300' },
      interruption: { high: 'bg-red-500', medium: 'bg-red-400', low: 'bg-red-300' },
      closing: { high: 'bg-green-500', medium: 'bg-green-400', low: 'bg-green-300' },
      rapport: { high: 'bg-purple-500', medium: 'bg-purple-400', low: 'bg-purple-300' },
      objection: { high: 'bg-orange-500', medium: 'bg-orange-400', low: 'bg-orange-300' },
    }
    ...
    <div className="glass rounded-xl p-6">
      ...
      <div className="relative mb-6">
        <div className="h-12 bg-gray-800 rounded-lg relative overflow-hidden">
          {sortedPoints.map((point, index) => {
            const position = (parseTimeToSeconds(point.timestamp) / duration) * 100
            return (
              <motion.div
                key={index}
                className={`absolute top-0 h-full w-1 ${getPointColor(point.type, point.importance)} cursor-pointer`}
                style={{ left: `${position}%` }}
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-3">
                <div className={`p-2 rounded ${getPointColor(point.type, point.importance)}`}>
