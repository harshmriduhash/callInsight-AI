...
export default async function handler(
  ...
  try {
    ...
    if (!transcription || typeof transcription !== 'string' || transcription.trim().length < 20) {
...
- suggestion: string com sugestão rápida (máximo 50 caracteres)`
    ...
    console.log('📤 Enviando requisição para GPT-Audio-Mini com', transcription.length, 'caracteres')
    ...
    try {
      ...
      console.warn('⚠️ Erro ao parsear JSON diretamente, tentando extrair:', parseError.message)
      ...
      if (jsonMatch) {
        try {
          ...
          throw new Error(`Erro ao parsear resposta JSON: ${extractError.message}. Resposta recebida: ${analysisText.substring(0, 200)}`)
        }
      } else {
        console.error('❌ Nenhum JSON encontrado na resposta')
        throw new Error(`Não foi possível encontrar JSON na resposta. Resposta recebida: ${analysisText.substring(0, 200)}`)
      }
    }
    
    // Validar campos obrigatórios
    if (typeof analysis.sentiment !== 'number' || typeof analysis.engagement !== 'number') {
      console.error('❌ Resposta inválida - campos faltando:', analysis)
      throw new Error('Resposta da API não contém campos sentiment ou engagement válidos')
    }
